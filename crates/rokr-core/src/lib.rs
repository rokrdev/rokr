...
pub trait Provider {
    ...
    async fn send(&self, messages: &[Message]) -> Result<Message, Self::Error>;
}
...
/// the provider with just that one message, and hand back whatever
...
mod tests {
    ...
    impl Provider for StubProvider {
        ...
        async fn send(&self, messages: &[Message]) -> Result<Message, StubError> {
